...
void initialise()
{
    ...
    g_display_window = SDL_CreateWindow("ALIEN SHOOTER!",
    ...
    while (SDL_PollEvent(&event))
    {
        ...
        case SDL_KEYDOWN:
            ...
            case SDLK_SPACE:
                ...
                if (g_current_scene->m_state.player->m_animation_indices == g_current_scene->m_state.player->m_walking[g_current_scene->m_state.player->RIGHT])
                {
                    g_current_scene->m_state.bullet->m_movement.x = 1;
                    g_current_scene->m_state.bullet->m_movement.y = 0;
                }
                else if (g_current_scene->m_state.player->m_animation_indices == g_current_scene->m_state.player->m_walking[g_current_scene->m_state.player->LEFT])
                {
                    g_current_scene->m_state.bullet->m_movement.x = -1;
                    g_current_scene->m_state.bullet->m_movement.y = 0;
                }
                else if (g_current_scene->m_state.player->m_animation_indices == g_current_scene->m_state.player->m_walking[g_current_scene->m_state.player->UP])
                {
                    g_current_scene->m_state.bullet->m_movement.y = 1;
                    g_current_scene->m_state.bullet->m_movement.x = 0;
                }
                else if (g_current_scene->m_state.player->m_animation_indices == g_current_scene->m_state.player->m_walking[g_current_scene->m_state.player->DOWN])
                {
                    g_current_scene->m_state.bullet->m_movement.y = -1;
                    g_current_scene->m_state.bullet->m_movement.x = 0;
                }
...
void update()
{
    ...
    while (delta_time >= FIXED_TIMESTEP) {
        ...
        if (g_current_scene->end_game == 1) {
            switch_to_scene(g_level_2);
            g_current_scene->end_game = 3;

        }
        if (g_current_scene->end_game == 2) {
            switch_to_scene(g_level_2);
            g_current_scene->end_game = 4;
    ...
    // ————— PLAYER CAMERA ————— //
    g_view_matrix = glm::mat4(1.0f);
    g_view_matrix = glm::translate(g_view_matrix, glm::vec3(-g_current_scene->m_state.player->get_position().x, 3.5f, 0.0f));
